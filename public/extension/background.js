...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.type === 'SCAN_PAGE') {
    ...
    handleScan(request.text, request.url).then(sendResponse);
  ...
  if (request.type === 'SET_CONFIG') {
    ...
    USER_ID = request.uid;
    chrome.storage.local.set({ appUrl: APP_URL, uid: request.uid });
    connectWebSocket();
  }
});

// Initialize from storage
chrome.storage.local.get(['appUrl', 'uid'], (result) => {
  APP_URL = result.appUrl ? result.appUrl.replace(/\/$/, '') : null;
  USER_ID = result.uid;
  if (APP_URL) connectWebSocket();
});

async function handleScan(text, url) {
  if (!APP_URL || !USER_ID) {
    ...
    return { maladies: [], error: errorMsg };
  }
  ...
  try {
    ...
    relayLog(`Received response from server. Found ${data.maladies?.length || 0} maladies.`);
    
    if (data.error) {
      relayLog(`API Error: ${data.error}`, 'error');
      return { maladies: [], error: data.error };
    }
    
    return { maladies: data.maladies || [] };
  } catch (error) {
    relayLog(`Fetch Error: ${error.message}`, 'error');
    return { maladies: [], error: 'Network connection failed.' };
  }
}
